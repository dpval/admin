...
export async function fetchClientPosts() {
  ...
  try {
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      // Fetch posts for each client with status "Approved"
      const postQuery = query(postTaskCollection, where("status", "==", "Approved"));
      ...
      postTaskSnapshot.forEach(async (postDoc) => {
        ...
        if (timeDiff > 48) {
          ...
          // Update the status to "Archived" in Firestore
          const postRef = doc(db, `users/${userDoc.id}/postTask`, postDoc.id);
